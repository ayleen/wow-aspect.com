...
if ($get_prop == 'char')
{
	...
	if ($result)
	{
		...
		print stripslashes($result['tooltip']);
	}
	else
	{
		print "Информация о персонаже не найдена.<br/>Возможно истекло время хранения кэша, попробуйте обновить страницу.<br/>Ключ кэша: $uniquekey<br/>Имя персонажа: $get_char<br/>Сервер: $get_realm<br/>Регион: $get_region";
	}
}
...
if ($get_prop == 'guild')
{
	...
	{
		print "Информация о персонаже не найдена.<br/><strong>Ключ:</strong> " . $uniquekey;
	}
}
$config->close();
$cache->close();
