...
elseif ($mode == 'raid')
{
	...
	if (@mysql_num_rows($query) == 0 || trim($raid) == '')
	{
		...
		{
			$out .= '<strong>10 игроков</strong><br />';
		...
		{
			$out .= '<br/><strong>25 игроков</strong><br />';
...
elseif ($mode == 'talents')
{
	...
	if (mysql_num_rows($query) == 0 || trim($result) == '')
	{
		...
		if (sizeof($specs['spec']) > 1)
		{
			...
			if (!$pt_used)
				$out .= '<div align="center" style="padding-top: 5px;">Нет умений, используемых в этой ветке.</div>';
		...
		if (array_key_exists('glyphs', $specs['spec'][1]) || array_key_exists('glyphs', $specs['spec'][2]))
		{
			...
			$out .= '<tr><td class="glyph-table-head">Символы</td></tr>';
			...
			if (array_key_exists('glyphs', $specs['spec'][1]))
			{
				...
				{
					$out .= '<div class="glyph-title">Большие</div>';
				...
				{
					$out .= '<div class="glyph-title">Малые</div>';
