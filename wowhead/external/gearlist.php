...
$name = stripslashes(html_entity_decode($_GET['name'], ENT_QUOTES, "UTF-8"));
$realm = stripslashes(html_entity_decode($_GET['realm'], ENT_QUOTES, "UTF-8"));
$region = stripslashes(html_entity_decode($_GET['region'], ENT_QUOTES, "UTF-8"));
if ($name == '')
{
...
if (mysql_num_rows($query) == 0)
{
	...
	{
		print 'Не удалось получить XML. Возможно, доступ к Оружейной блокирован.';	
	}
	else
	{
		//print_r($xml); exit;
		...
		{
			print 'Персонаж не был найден либо возникла иная проблема.';
			mysql_close($conn);
			exit;	
		}
