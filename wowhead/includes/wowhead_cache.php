...
class wowhead_cache
{
	...
	public function getArmory($uniquekey, $max_age)
	{
		...
		$query_text = 'SELECT
						...
						WHERE
							uniquekey=\'' . $uniquekey . '\'
							AND cache > UNIX_TIMESTAMP(NOW()) - ' . (int)$max_age . '
						LIMIT 1';
		//var_dump($query_text, $max_age); die;
	...
	public function getClass($name, $lang)
	{
		if (trim($name) == '' || !$this->connected)
			return false;
		$name = addslashes($name);
		$query_text = "SELECT id, name, icon, tooltip, lang FROM `" . WHP_DB_PREFIX . "class` WHERE (search_name LIKE '{$name}' AND lang='{$lang}') LIMIT 1";
		$this->sql->query($query_text);
		return ($this->sql->num_rows($this->sql->query_id) == 0) ? false : $this->sql->fetch_record($this->sql->query_id);
	}
	...
	private function prepareInfoArray($array)
	{
		...
		{
			if ($key == 'name' || $key == 'search_name' || $key == 'icon' || $key == 'pattern' || $key == 'tooltip')
